        ...
        ,done: function(page, next){ //加载下一页
            ...
            $.get('/api/image/list?page='+page+'&limit=9', function(res){
                ...
                layui.each(res.data, function(index, item){
                    lis.push('<div class="column"  style="cursor: pointer;" onclick="window.open(\'/api/image/view/'+item.id+'\',\'_self\')">\n' +
                        '                    <div class="ui fluid card">\n' +
                        '                        <div class="image">\n' +
                        '                            <img lay-src="'+item.filePath+'">\n' +
                        '                        </div>\n' +
                        '                        <div class="content">\n' +
                        '                            <a class="header" href="/api/image/view/'+item.id+'">'+item.fileName+'</a>\n' +
                        '                        </div>\n' +
                        '                    </div>\n' +
                        '                </div>')
                });
